import { Box, Button, Container, Grid, Typography } from "@material-ui/core";
import React, { Component } from "react";
import { RenderSectionHeading } from "../common/commonComponent";
import { useStyles } from "./BodyStyles";
import ScrollAnimation from "react-animate-on-scroll";
import "./Team.css";
import { teamData } from "./../HomePage/data";

export default function Team2() {
  const classes = useStyles();

  const clickFunc = (e) => {
    //   console.log(e.target.getAttribute('key'))
    console.log(e.target.getAttribute("alt"));
    const dataKey = e.target.getAttribute("alt");
    const clickdata = [...teamData];
    const myData = clickdata[dataKey].nameData;
    console.log(clickdata[dataKey].nameData);
    // return(
    //     <Box className="info-container">
    //     <img className="imgone" src={clickdata[dataKey].image} alt="" />
    //     <Box className="info">
    //       <Typography variant="h1" className="name" id="name">
    //         {clickdata[dataKey].nameData}{" "}
    //       </Typography>
    //       <Typography variant="h5" className="status" id="status">
    //         {clickdata[dataKey].statusData}
    //       </Typography>
    //       <Typography paragraph className="about" id="about">
    //         {clickdata.aboutData}
    //       </Typography>
    //     </Box>
    //   </Box>
    // )
    

    // teamData.map((item) => (
    //   <Box className="info-container">
    //     <img className="imgone" src={item.image} alt="" />
    //     <Box className="info">
    //       <Typography variant="h1" className="name" id="name">
    //         {item.nameData}{" "}
    //       </Typography>
    //       <Typography variant="h5" className="status" id="status">
    //         {item.statusData}
    //       </Typography>
    //       <Typography paragraph className="about" id="about">
    //         {item.aboutData}
    //       </Typography>
    //     </Box>
    //   </Box>
    // ));
  };

  return (
    <Box className={classes.sectionDark} id="Team">
      <ScrollAnimation animateIn="fadeIn">
        <Grid
          container
          style={{
            displa: "flex",
            justifyContent: "center",
            alignContent: "center",
          }}
        >
          <Grid item xs={12} sm={8}>
            {RenderSectionHeading({
              smallText: "Team",
              heading: "Let's See Us",
              alignCenter: true,
            })}
          </Grid>
        </Grid>

        {/* TEAM DETAILS */}

        <Box className="bodydiv">
          <Box className="image-container">
            <img
              className="imageall"
              src="data:image/jpeg;base64,"
            ></img>
          </Box>
          <Box className="info-container">
            <img
              className="imgone"
              src="data:image/jpeg;base64,"
            />
            <Box className="info">
              <Typography variant="h1" className="name" id="name">
                Rajib Ghosh{" "}
              </Typography>
              <Typography variant="h5" className="status" id="status">
                Director
              </Typography>
              <Typography paragraph className="about" id="about">
                Lorem ipsum dolor sit amet, consectetur adipisicing elit. Et
                voluptates placeat cumque dolores, eaque saepe, maiores corrupti
                officiis labore iusto consectetur delectus repudiandae error
                incidunt rem? Adipisci odio error minima.
              </Typography>
            </Box>
          </Box>
        </Box>

        {/* Second */}
        <Box className="bodydiv">
          <Box className="image-container">
            {teamData.map((item) => (
              <img
                className="imageall"
                src={item.image}
                alt={item.id}
                type={item.id}
                key={item.id}
                onClick={clickFunc}
              />
            ))}
          </Box>
          <Box>
                {/* {clickFunc.clickdata[clickFunc.dataKey].nameData} */}
                {/* <Box className="info-container">
                <img className="imgone" src={clickFunc.clickdata[clickFunc.dataKey].image} alt="" />
                <Box className="info">
                  <Typography variant="h1" className="name" id="name">
                    {clickFunc.clickdata[clickFunc.dataKey].nameData}{" "}
                  </Typography>
                  <Typography variant="h5" className="status" id="status">
                    {clickFunc.clickdata[clickFunc.dataKey].nameData}
                  </Typography>
                  <Typography paragraph className="about" id="about">
                    {clickFunc.clickdata[clickFunc.dataKey].nameData}
                  </Typography>
                </Box>
              </Box> */}
              
          </Box>

                {/* {console.log("HI")} */}
          {/* {console.log(clickdata[dataKey].nameData)} */}
          {/* {console.log(clickFunc.clickdata[clickFunc.dataKey].nameData)} */}

          {/* {teamData.map((item) => (
              <Box className="info-container">
                <img className="imgone" src={item.image} alt="" />
                <Box className="info">
                  <Typography variant="h1" className="name" id="name">
                    {item.nameData}{" "}
                  </Typography>
                  <Typography variant="h5" className="status" id="status">
                    {item.statusData}
                  </Typography>
                  <Typography paragraph className="about" id="about">
                    {item.aboutData}
                  </Typography>
                </Box>
              </Box>
            ))} */}
          {/* </Box> */}
        </Box>
        {/* done */}
      </ScrollAnimation>
    </Box>
  );
}
